...
def key_down(hwnd, key: chr):
    vk = VK_CODE.get(key)
    scancode = win32api.MapVirtualKey(vk,0)
    win32gui.PostMessage(hwnd, win32con.WM_ACTIVATE, win32con.WA_ACTIVE, 0)
    win32api.PostMessage(hwnd, win32con.WM_KEYDOWN, vk, int(scancode<<16|1))

def key_up(hwnd, key: chr):
    vk = VK_CODE.get(key)
    scancode = win32api.MapVirtualKey(vk,0)
    win32gui.PostMessage(hwnd, win32con.WM_ACTIVATE, win32con.WA_ACTIVE, 0)
    win32api.PostMessage(hwnd, win32con.WM_KEYUP, vk, int(scancode<<16|1|0xC0000000))

def click_key(hwnd, key: chr):
    key_down(hwnd, key)
    key_up(hwnd, key)

def click_pos(hwnd, pos: tuple):
